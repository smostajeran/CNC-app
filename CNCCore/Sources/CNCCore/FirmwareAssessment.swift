...
public struct FirmwareAssessment: Equatable, Sendable {
    ...
    public static func assess(
        ...
        if let xTravel = settings["$130"], let yTravel = settings["$131"] {
            if xTravel < 100 || yTravel < 80 || xTravel > 500 || yTravel > 400 {
                notes.append(String(format: "Travel $130/$131 = %.0f×%.0f mm — unusual for TA-4 (~390×200); verify soft limits.", xTravel, yTravel))
            } else {
        ...
        if isGrbl && is11 {
            notes.append("Looks like GRBL 1.1 — suitable for Quill ok-paced streaming and $J= jog.")
            return FirmwareAssessment(
    ...
    /// Pulls a version token such as `1.1f` or `1.1z` from `$I` / banner text.
